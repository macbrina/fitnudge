...
import { onboardingApi, type SuggestedGoalsStatusResponse } from "@/services/api/onboarding";
...
export const useSuggestedGoalsStatus = (
  ...
  return useQuery({
    ...
    refetchInterval: (query) => {
      const status = query.state.data?.status ?? (query.state.data as any)?.status;
...
export type GoalTypeOption = "habit" | "time_challenge" | "target_challenge" | "mixed";
...
export const useRequestSuggestedGoals = () => {
  ...
  return useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData<SuggestedGoalsStatusResponse>(suggestedGoalsQueryKeys.status(), {
        status: "pending"
      });
    },
    onSuccess: (data) => {
