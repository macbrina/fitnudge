...
interface CustomGoalScreenProps {
  onContinue: (details: CustomGoalDetails) => void;
  onBack?: () => void;
  currentStep: number;
  totalSteps: number;
}

export interface CustomGoalDetails {
  title: string;
  frequency: number;
  isDaily: boolean;
...
export default function CustomGoalScreen({
  ...
  currentStep,
  totalSteps
}: CustomGoalScreenProps) {
  // Get initial values from store (persisted)
  const storedTitle = useOnboardingStore((state) => state.goal_title);
  ...
  // Can continue: title is valid AND (daily OR days match frequency) AND has reminder time
  const hasReminderTime = reminderTimes.length > 0;
  const canContinue = title.trim().length >= 3 && isDaysValid && hasReminderTime;
  ...
  return (
    ...
    >
      <View style={styles.content}>
        <Text style={styles.title}>{t("onboarding.custom_goal.title")}</Text>

        {/* Goal Title */}
        <TextInput
          placeholder={t("onboarding.custom_goal.placeholder")}
          value={title}
        ...
        <View style={styles.sectionCard}>
          {/* Frequency Type */}
          <Text style={styles.label}>{t("onboarding.custom_goal.how_often")}</Text>
