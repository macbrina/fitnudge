...
const TARGET_OPTIONS = [
  { value: 25, label: "25 check-ins" },
  { value: 50, label: "50 check-ins" },
  { value: 75, label: "75 check-ins" },
  { value: 100, label: "100 check-ins" },
];
            ...
            <View style={styles.optionsGrid}>
              {CHALLENGE_DURATIONS.map((duration) => (
                <Button
                  ...
                  onPress={() => setChallengeDuration(duration.value)}
                  variant={
                    challengeDuration === duration.value ? "primary" : "outline"
                  }
                  size="sm"
                  borderRadius="lg"
                  style={styles.optionButton}
            ...
            <View style={styles.optionsGrid}>
              {TARGET_OPTIONS.map((target) => (
                <Button
                  key={target.value}
                  title={target.label}
                  onPress={() => setTargetCheckins(target.value)}
                  variant={
                    targetCheckins === target.value ? "primary" : "outline"
                  }
                  size="sm"
                  borderRadius="lg"
                  style={styles.optionButton}
