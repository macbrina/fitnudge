...
export function LeaderboardPreview({
  ...
  const getRankDisplay = (rank: number) => {
    if (rank <= 3) {
      return (
        <View style={[styles.medalContainer, { backgroundColor: getMedalColor(rank) }]}>
    ...
    return (
      <Card shadow="md" style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.sectionTitle}>{t("challenges.leaderboard") || "Leaderboard"}</Text>
        </View>
        {[1, 2, 3].map((i) => (
      ...
      <Card shadow="md" style={styles.container}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <Ionicons name="podium-outline" size={20} color={brandColors.primary} />
            <Text style={styles.sectionTitle}>{t("challenges.leaderboard") || "Leaderboard"}</Text>
          </View>
        </View>
        ...
        <View style={styles.emptyState}>
          <View style={styles.emptyIconContainer}>
            <Ionicons name="trophy-outline" size={40} color={colors.text.tertiary} />
          </View>
          <Text style={styles.emptyTitle}>
            {t("challenges.no_participants_yet") || "No Participants Yet"}
          </Text>
          <Text style={styles.emptyText}>
            {t("challenges.be_first_to_join") || "Be the first to join and claim the top spot!"}
          </Text>
        </View>
      </Card>
    );
      ...
      <View style={styles.entriesContainer}>
        {entries.map((entry, index) => {
          ...
          return (
            ...
            >
              {/* Rank */}
              <View style={styles.rankContainer}>{getRankDisplay(entry.rank)}</View>

              {/* Avatar */}
              <View style={styles.avatarContainer}>
                {entry.user?.profile_picture_url ? (
                  <Image source={{ uri: entry.user.profile_picture_url }} style={styles.avatar} />
                ) : (
              ...
              <View style={styles.entryContent}>
                ...
                <Text style={styles.checkInsCount}>
                  {entry.total_check_ins} {t("checkin.check_ins") || "check-ins"}
                </Text>
              </View>
...
const makeLeaderboardPreviewStyles = (tokens: any, colors: any, brand: any) => ({
  container: {
