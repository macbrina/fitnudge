...
async function proxy(
  ...
  const url = `${getAdminApiUrl()}/api/broadcasts${pathStr}${query ? `?${query}` : ""}`;
  ...
  try {
    ...
    console.error("Broadcasts proxy error:", e);
...
export async function POST(
  ...
  const { path = [] } = await params;
  if (path.length > 0) {
    return NextResponse.json({ detail: "POST to list only" }, { status: 400 });
  }
...
export async function PUT(
  ...
  const { path = [] } = await params;
  if (path.length === 0) {
    return NextResponse.json({ detail: "ID required for update" }, { status: 400 });
  }
...
export async function DELETE(
  ...
  const { path = [] } = await params;
  if (path.length === 0) {
    return NextResponse.json({ detail: "ID required for delete" }, { status: 400 });
  }
