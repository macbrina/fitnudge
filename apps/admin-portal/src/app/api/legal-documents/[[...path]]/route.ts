...
async function proxy(
  ...
  const url = `${getAdminApiUrl()}/api/legal-documents${pathStr}${query ? `?${query}` : ""}`;
  ...
  try {
    ...
    console.error("Legal documents proxy error:", e);
...
export async function PUT(
  ...
  if (path.length === 0) {
    return NextResponse.json(
      { detail: "Document ID required for update" },
      { status: 400 }
    );
  }
...
export async function DELETE(
  ...
  if (path.length === 0) {
    return NextResponse.json(
      { detail: "Document ID required for delete" },
      { status: 400 }
    );
  }
