...
import { useTasksScheduled, useTaskRevoke } from "@/hooks/api/useTasks";
...
export function TasksScheduledView() {
  ...
  const { data, isLoading, isError } = useTasksScheduled();
  ...
  const handleRevoke = async (taskId: string) => {
    try {
      await showConfirm({
        ...
        onConfirm: async () => {
          await revokeMutation.mutateAsync({ id: taskId });
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-x-auto">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-gray-500 dark:text-gray-400">
            <p className="font-medium">{t("admin.tasks.no_scheduled")}</p>
            <p className="text-sm mt-1">{t("admin.tasks.no_scheduled_hint")}</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t("admin.tasks.col_eta")}</TableHead>
            ...
            <TableBody>
              {tasks.map((task) => (
                <TableRow key={task.id}>
                  ...
                  <TableCell>
                    {task.eta
                      ? new Date(task.eta).toLocaleString()
                      : "—"}
                  </TableCell>
                  <TableCell>
                    <Link
