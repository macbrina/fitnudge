...
import { useAnalyticsCheckinActivity } from "@/hooks/api/useAnalytics";
...
export function AnalyticsCheckinsView() {
  const { t } = useTranslation();
  const [days, setDays] = useState<number>(30);
  const { data, isLoading, error, refetch } = useAnalyticsCheckinActivity(days);
  ...
  return (
    <div className="space-y-6">
      <p className="text-gray-600 dark:text-gray-400">
        {t("admin.pages.checkin_activity_description")}
      </p>

      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-lg font-semibold">{t("admin.menu.checkin_activity")}</h2>
        <div className="flex gap-2">
          <Button
            ...
            onClick={() =>
              data?.data &&
              exportToCsv(data.data, `checkin-activity-${days}d.csv`, [
                "date",
                "completed",
                "missed",
                "total",
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.data}>
                  ...
                  <Bar dataKey="missed" stackId="a" fill="#ef4444" name={t("admin.analytics.missed")} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
